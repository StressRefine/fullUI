...
namespace SRUi
{
    public partial class chart : Form
    {
        ...
        public new DialogResult ShowDialog()
        {
            ...
            if (mainform.stressCnvType == 0)
                stressPassTypeString = "Max Von Mises Stress";
            else if (mainform.stressCnvType == 1)
                stressPassTypeString = "Max Principal Stress";
            else
                stressPassTypeString = "Max Custom Stress Criterion";
            ...
            for (int i = 0; i < mainform.numStressPasses; i++)
            {
                if (mainform.stressCnvType == 0)
                    ...
                    stress[i] = mainform.sp1PassVec[i];
                else
                    stress[i] = mainform.stressCustomPassVec[i];
                if (stress[i] < smin)
                    smin = stress[i];
                if (stress[i] > smax)
                    smax = stress[i];
            ...
            CnvGraph.Titles["Title1"].Text = stressPassTypeString + System.Environment.NewLine + StressUnitString;
            CnvGraph.ChartAreas["ChartArea1"].AxisY.Title = stressPassTypeString;
